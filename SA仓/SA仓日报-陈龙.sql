...
select * from ( select '${data_date}' as data_date ) a join (
    ...
    select
        ...
        decode(c.总付款单数,0,0,c.全SA仓付款订单/c.总付款单数) as sa_paid_orders_ratio,
        ...
        decode(c.全SA仓订单,0,0,c.全SA仓付款订单/c.全SA仓订单) as sa_order_paid_rate,
        ...
        c.全SA仓付款订单 as sa_paid_orders,
        c.全CN仓付款订单 as cn_paid_orders,
        c.混合仓付款订单 as mix_paid_orders,

        c.全SA仓订单 as sa_order,
        c.全SA仓付款订单 as sa_order_paid,
    ...
    from (
        select
            ...
            sum(case when b.depot_coverage_area_id=2 and b.source_order_id=0 and a.is_split is null then 1 else 0 end)全SA仓订单,
            ...
            sum(case when a.pay_status in(1,3) and b.depot_coverage_area_id=2 and b.source_order_id=0 and a.is_split is null then 1 else 0 end)全SA仓付款订单,
        ...
        from ( select add_time,order_id,is_split,pay_status from zydb.dw_order_fact where unix_timestamp(add_time) >= unix_timestamp('${data_date}','yyyyMMdd') and unix_timestamp(add_time) < unix_timestamp('${data_date}','yyyyMMdd')+86400 and country_name='Saudi Arabia' and site_id in (600,900)
    ...
    from (
        ...
        from (--付款订单数
            ...
            from (
                ...
                where a.pay_status=1 and a.depod_id=7 and unix_timestamp(a.add_time)>=unix_timestamp('${data_date}','yyyyMMdd')-(86400*10)
            )a join (
    ...
    join (
        ...
        from (--总发运订单数
            ...
            from (
                ...
                from zydb.dw_order_sub_order_fact
                where pay_status=1 and depod_id=7 and unix_timestamp(shipping_time)>=unix_timestamp('${data_date}','yyyyMMdd') and unix_timestamp(shipping_time)<unix_timestamp('${data_date}','yyyyMMdd')+86400
            )a
        )a join (
            select
    ...
    join (
        select
            a.total_SA_canl_rate,          --全SA仓取消率
            b.mix_SA_canl_rate,            --混SA仓取消率
            c.total_CN_canl_rate,          --全CN仓取消率
            d.mix_CN_canl_rate,            --混CN仓取消率

            a.total_SA_canl_number,
            a.total_SA_canl_order_number,
            b.mix_SA_canl_number,
            b.mix_SA_canl_order_number,
