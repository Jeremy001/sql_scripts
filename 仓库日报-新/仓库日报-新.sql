...
--分配订单量、单量占比
insert overwrite table rpt.rpt_depot_daily_report_new_tmp1
select 
  t0.depot_id,
  orders_num,
  prop_orders,
...
(
	...
	from dw.dw_delivered_receipt_onself a
...
(
	--上架时长 
    	...
    	(
      		...
      		from dw.dw_order_node_time a
      		left join  dw.dw_demand_pur b on a.order_id=b.order_id
      		left join  dw.dw_delivered_receipt_onself c 
...
--提货订单数
	...
	dw.dw_order_node_time a
	left join  
	dw.dw_order_sub_order_fact b
...
(
	...
	select depod_id depot_id,sum(unix_timestamp(b.lading_time)-unix_timestamp(a.shipping_time))/3600/count(*) pickup_time
	 from
	dw.dw_order_sub_order_fact a
	left join  
	dw.dw_order_node_time  b
	on a.order_id=b.order_id
	where lading_time>=from_unixtime(unix_timestamp('${data_date}','yyyyMMdd'))
	and lading_time<date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1)
	group by depod_id
...
insert overwrite table rpt.rpt_depot_daily_report_new_tmp2
select 
  t0.depot_id,
...
(
	...
	from  ods.ods_who_wms_goods_stock_detail a
...
select a.depot_id,
  ...
  from
ods.ods_who_wms_goods_stock_total_detail  a
    where   data_date='${data_date}'
group by a.depot_id
)t10
...
insert overwrite table rpt.rpt_depot_daily_report_new_tmp3
select 
   t0.depot_id,
...
insert overwrite table rpt.rpt_depot_daily_report_new_tmp4
select 
	t0.depot_id,
    exp_goods_num,
...
set hive.exec.parallel=true;
---仓库日报
insert overwrite table rpt.rpt_depot_daily_report_new
select 
  ${data_date} data_date,
...
from 
rpt.rpt_depot_daily_report_new_tmp1 t1
full join 
rpt.rpt_depot_daily_report_new_tmp2 t2
on t1.depot_id=t2.depot_id
full join 
rpt.rpt_depot_daily_report_new_tmp3 t3
on t1.depot_id=t3.depot_id
full join 
rpt.rpt_depot_daily_report_new_tmp4 t4
on t1.depot_id=t4.depot_id
left join dim.dim_dw_depot t5 on t1.depot_id=t5.depot_id
where 
t5.status=1

union all

select * from rpt.rpt_depot_daily_report_new
where data_date<>${data_date}
...
--分配订单量、单量占比
insert overwrite table rpt.rpt_depot_daily_report_new_total
select 
  '${data_date}' data_date,
  receipt_quality_duration,
  quality_onshelf_duration,
  ...
  package_duration,
  shipping_duration,
  picking_duration+package_duration+shipping_duration picking_shiping_duration,  
  pickup_time
...
(
	--质检时长
	select 
       sum(((unix_timestamp(on_shelf_start_time)-unix_timestamp(start_receipt_time))/3600)*num)/sum(num)  as receipt_quality_duration
	from dw.dw_delivered_receipt_onself a
	 where a.on_shelf_start_time >=from_unixtime(unix_timestamp('${data_date}','yyyyMMdd'))
	   and a.on_shelf_start_time < date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1)
	   and ((unix_timestamp(on_shelf_start_time)-unix_timestamp(start_receipt_time))/3600/24)>0
...
(
	--提货时长 
	select sum(unix_timestamp(b.lading_time)-unix_timestamp(a.shipping_time))/3600/count(*) pickup_time
	 from
	dw.dw_order_sub_order_fact a
	left join  
	dw.dw_order_node_time  b
...
select * from rpt.rpt_depot_daily_report_new_total
where data_date<>${data_date}
