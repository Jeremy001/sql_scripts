...
(
	...
	select
		 depot_area,
		 sum(unix_timestamp(receipt_time )-unix_timestamp(pay_time))/count(*)/3600 lt04
	 from
	zydb.dw_order_shipping_tracking_node a
	left join zydb.dw_order_node_time b on a.order_id=b.order_id
	inner join zydb.dim_dw_depot c on b.depot_id=c.depot_id
	where c.depot_area  in('cn','sa')
	and to_date(receipt_time)=to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
...
select  depot_area,sum(
	...
	from (
		  select  b.rec_id,lock_org_num,lock_oos_num,case when a.pay_id=41 then a.pay_time else result_pay_time end   pay_time,d.depot_area,min(nvl(c.start_receipt_time,c.first_check_time)) start_receipt_time,min(c.on_shelf_start_time) on_shelf_start_time
		  from
      ...
      on a.depod_id=d.depot_id
		  where
		  least(nvl(start_receipt_time,c.first_check_time),on_shelf_start_time)>=from_unixtime(unix_timestamp('${data_date}','yyyyMMdd'))
		  and least(nvl(start_receipt_time,c.first_check_time),on_shelf_start_time)<date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1)
		  and b.demand_type in(1,2,7)
...
--物流时长(LT34)
	select
		 depot_area,
		 sum(unix_timestamp(receipt_time )-unix_timestamp(b.shipping_time))/count(*)/3600 LT34
	 from
	zydb.dw_order_shipping_tracking_node a
	left join zydb.dw_order_node_time b on a.order_id=b.order_id
	inner join zydb.dim_dw_depot c on b.depot_id=c.depot_id
...
where data_date>=regexp_replace(date_sub(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),6),'-','')
and data_date<regexp_replace(date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1),'-','')
and depot_area in('cn','sa')
group by depot_area
...
where a.is_shiped=1
and depot_area in('cn','sa')
and to_date(a.shipping_time)>=to_date(date_sub(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),6))
and to_date(a.shipping_time)<to_date(date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1))
group by depot_area
...
--create table zydb.dpr_supply_chain_final_table_new2 as

insert overwrite table zydb.dpr_supply_chain_final_table_new2
select t1.depot_area,depot_link_mtd,receive_mtd
from
(
select 'all_depot' depot_area,
       sum(nvl(pur_shiped_order_onway,0)+
       nvl(total_stock,0)+
       nvl(return_onway,0)+
       nvl(ship_onway,0)) depot_link_mtd
 from zydb.dw_full_link_goods_detail
where data_date>=regexp_replace(date_sub(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),6),'-','')
and data_date<regexp_replace(date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1),'-','')
)t1
left join
...
--MTD签收数总和
     select 'all_depot' depot_area,
      ...
      from zydb.dw_order_sub_order_fact a
    inner join zydb.dw_order_shipping_tracking_node b on a.order_id=b.order_id
    inner join zydb.dw_order_goods_fact c on a.order_id=c.order_id
    inner join  zydb.dim_dw_depot d on a.depod_id=d.depot_id
                                    and depot_area<>''
    where substr(regexp_replace(receipt_time,'-',''),1,6) = substr('${data_date}',1,6)
	and to_date(receipt_time)>= to_date(date_sub(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),6))
	and to_date(receipt_time)<to_date(date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1))
)t2
on t1.depot_area=t2.depot_area
;
...
--create table zydb.dpr_supply_chain_final_table_new3 as

insert overwrite table zydb.dpr_supply_chain_final_table_new3
select *
from
(
--按需采购反应时长(LT0)
...
--商品到货时长(LT1)
	select  sum(
				(
					unix_timestamp(least(nvl(start_receipt_time,on_shelf_start_time),on_shelf_start_time))-unix_timestamp(push_gmt_created)
				)
				*(lock_org_num-lock_oos_num))/sum(lock_org_num-lock_oos_num) pur_duration
	from (
		  select  b.rec_id,lock_org_num,b.push_gmt_created,lock_oos_num,min(c.start_receipt_time) start_receipt_time,min(c.on_shelf_start_time) on_shelf_start_time
		  from
		  zydb.dw_demand_pur    b
		  left join
		  zydb.dw_delivered_receipt_onself   c
		  ...
		  and least(start_receipt_time,on_shelf_start_time)<date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1)
		  and b.demand_type=1
		  group by b.rec_id,lock_org_num,lock_oos_num,b.push_gmt_created
	)a
)t2,
(
--仓库作业时长(LT2) 质检时长+上架时长+拣货时长+打包时长+发运时长
    ...
    --质检时长
      select
       sum(((unix_timestamp(on_shelf_start_time)-unix_timestamp(start_receipt_time))/3600)*num)/sum(num)  as check_duration
	  ...
	  where a.on_shelf_start_time >=from_unixtime(unix_timestamp('${data_date}','yyyyMMdd'))
	   ...
	   and ((unix_timestamp(on_shelf_start_time)-unix_timestamp(start_receipt_time))/3600)>0
    )t1,
    (
    	--上架时长
    	select
...
(
  --转运时长(LT3)
  select
  sum(unix_timestamp(arrive_time) -unix_timestamp(shipping_time))/count(*)/3600 lt3
...
(
  --配送时长(LT4)
  select
  sum(unix_timestamp(receipt_time) -unix_timestamp(arrive_time))/count(*)/3600 LT4
  from  zydb.dw_order_shipping_tracking_node
...
(
  --订单系统审核时长
	select  sum(check_time - unix_timestamp(case when a.pay_id=41 then a.pay_time else a.result_pay_time end))/count(*)/3600   pay_push_dur
	from zydb.dw_order_sub_order_fact  a
	inner join jolly.who_order_info  b
	on a.order_id=b.order_id
	where check_time >= unix_timestamp('${data_date}','yyyyMMdd')
	and check_time < unix_timestamp(date_add(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')),1),'yyyy-MM-dd')
...
(
  --订单签收时长
  select
  sum(unix_timestamp(receipt_time) -unix_timestamp(shipping_time))/count(*)/3600  receive_time
  from  zydb.dw_order_shipping_tracking_node
  where to_Date(receipt_time)=to_Date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
  and receipt_time is not null
)t9,
...
--create table zydb.dpr_supply_chain_final_table_new4 as

insert overwrite table zydb.dpr_supply_chain_final_table_new4
select *
from
(
    --近7天商品平均销量
...
--create table zydb.dpr_supply_chain_final_table_new5 as

insert overwrite table zydb.dpr_supply_chain_final_table_new5

select *
from
(
--付款订单数、取消订单数、已发货订单数、国内仓子单付款订单数
select sum(case when to_date(case when pay_id=41 then pay_time else result_pay_time end) = to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
                     and pay_status in(1,3) then 1 else 0 end) pay_order_num,
       sum(case when to_date(shipping_time) =to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
                     and is_shiped=1 then 1 else 0 end) shipping_order_num,
	    sum(case when to_date(case when pay_id=41 then pay_time else result_pay_time end) = to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
	                 and b.depot_area='cn' then 1 else 0 end)  pay_order_num_cn
...
--采购商品件数
  ...
  (
      ...
      where to_date(demand_gmt_created)=to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
		and demand_type in(1,2,3,6,7,11,12,14)
  )a
)t4,
(
  --缺货商品件数
...
from
(
	select on_shelf_num,gmt_created from  jolly.who_wms_on_shelf_goods_price
	union all
	select on_shelf_num,gmt_created from  jolly_wms.who_wms_on_shelf_goods_price
)a
...
--出库商品件数(销)
	select
	   sum(b.goods_number) out_goods_num
	from zydb.dw_order_sub_order_fact a
	left join zydb.dw_order_goods_fact b on a.order_id=b.order_id
	where to_date(shipping_time)=to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
)t8,
...
(
--库存命中数(CN)
  select sum(original_goods_number)-sum(demand_supp_num) hit_rate_depot
  from
  (
      select  a.order_id all_order_id,c.order_id pur_order_id,b.original_goods_number,demand_supp_num
      from zydb.dw_order_sub_order_fact a
      left join zydb.dw_order_goods_fact b on a.order_id=b.order_id
      left join
      (
          select distinct rec_id,order_id,goods_id,sku_id,demand_supp_num
          from
          zydb.dw_demand_pur
      ) c
      on b.order_id=c.order_id and b.goods_id=c.goods_id and b.sku_id=c.sku_id
...
,
(
	select count(*) cancel_order_num
	from zydb.dw_order_sub_order_fact a
	left join zydb.dim_dw_depot b on a.depod_id=b.depot_id
	where  order_status=3 and is_shiped<>1
	and to_date(case when pay_id=41 then a.pay_time else result_pay_time end) = to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
	and b.status=1
...
insert overwrite   zydb.dw_full_link_goods_detail partition(data_date='${data_date}')
select  nvl(ship_onway,0) ship_onway,
        nvl(pur_shiped_order_onway_num,0) pur_shiped_order_onway_num,
        nvl(total_stock_num,0) total_stock_num,
        nvl(return_onway,0) return_onway

from
(
	--MTD全链路库存总和
		--采购在途
	select sum(pur_shiped_order_onway_num) pur_shiped_order_onway_num
...
(
	--退货在途 ，订单表已退货且不在退货表里
	select sum(original_goods_number) return_onway  from
	(
		...
		where shipping_state in(6,8)
		  and from_unixtime(b.update_time,'yyyyMMdd')  <= '${data_date}'
	)a
	inner join
	zydb.dim_dw_depot b
	on a.depod_id=b.depot_id
	...
	(
		  ...
		  where  p1.returned_stock_num > 0
			   ...
			   --and substr(from_unixtime(p1.stock_end_time,'yyyyMMdd'),1,6) = substr('${data_date}',1,6)
...
(
	--发货在途  ，已发货未签收
select
		  sum(a.goods_number) ship_onway
	from zydb.dw_order_sub_order_fact a
		left join
		(select distinct order_id,shipping_state from jolly.who_order_shipping_tracking )c on a.order_id=c.order_id
		inner join  zydb.dim_dw_depot d
			  on a.depod_id=d.depot_id
			  and depot_area<>''
	where shipping_state not in (3, 6, 8, 13)
	    and a.is_shiped=1
			and to_date(shipping_time) <= to_date(from_unixtime(unix_timestamp('${data_date}','yyyyMMdd')))
)t4
on 1=1
